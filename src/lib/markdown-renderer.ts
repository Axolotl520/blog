...
const inlineMathExtension: any = {
	...
	tokenizer(src: string) {
		// Avoid block-math and require a closing '$' on the same line
		if (!src.startsWith('$') || src.startsWith('$$')) return

		// Basic inline math: $...$ (no newlines). This intentionally stays conservative.
		...
		const token: InlineMathToken = { type: 'inlineMath', raw, text }
		return token as any
	},
	renderer(token: InlineMathToken) {
...
const blockMathExtension: any = {
	...
	tokenizer(src: string) {
		...
		const raw = match[0]
		const text = match[1].trim()
		const token: BlockMathToken = { type: 'blockMath', raw, text }
		return token as any
	},
	renderer(token: BlockMathToken) {
		return katex.renderToString(token.text, {
			throwOnError: false,
			displayMode: true
		})
...
// Register math extensions once at module load.
